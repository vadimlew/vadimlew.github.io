...
function LEV(app_width, app_height) {
	...
	function setTransform(sprite, transform) {
		for (var prop in transform) {
			...
			if (typeof transform[prop] == 'object' && typeof sprite[prop] == 'object' && !Array.isArray(transform[prop]) && !Array.isArray(sprite[prop])) setTransform(sprite[prop], transform[prop]);
	...
	function makeDraggable(sprite) {
		...
		function mouseup() {
			...
			console.log('x:' + sprite.x + ', y:' + sprite.y, 'scale.x ' + sprite.scale.x);
		}

		function keydownListener(e) {
			if (e.key == ' ') sprite.scale.x *= -1;
		}
	...
	function ResizeManager() {
		...
		this.remove = function(obj) {			
			...
			if (p) data.portrait.splice(data.portrait.indexOf(p),1);
			if (l) data.landscape.splice(data.landscape.indexOf(l),1);
		}
