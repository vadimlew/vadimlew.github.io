/**************************************
* LEVjs v1.5.1 (28.11.21)
**************************************/

function LEV(app_width, app_height, backgroundColor=0x000000) {
	...
	else document.onload = ()=>{document.body.appendChild(app.view)};
	...
	this.loopAnimateMesh = createLoopAnimateMesh;
	this.setTransform = setTransform;
	this.globalPosOf = globalPosOf;	
	this.dev = dev;
	this.shape = {};
	this.shape.rect = createShapeRect;
	this.shape.roundRect = createShapeRoundRect;
	this.shape.circle = createCircle;
	this.resize = new ResizeManager();
	this.interact = app.renderer.plugins.interaction;

	function createContainer(transform) {
		var container = new PIXI.Container();
		if (transform) setTransform(container, transform);
		return container;
	}

	function createSprite(texture_name, transform) {
		if (texture_name instanceof PIXI.Texture) {
	...
	function createText(string, transform) {
		var text = new PIXI.Text(string);
		if (transform) setTransform(text, transform);
	...
	function createLoopAnimateMesh(mesh, data, timeOrTween) {
		var points = []; 
		var displacies = []; 
	...
	function createCircle(color, size, transform)	{
		var shape = new PIXI.Graphics();
		shape.beginFill(color);
		shape.drawCircle(...size);
		if (transform) setTransform(shape, transform);
		return shape;
	}	

	function createShapeRect(color, size, transform)	{
		var shape = new PIXI.Graphics();
		shape.beginFill(color);
		shape.drawRect(...size);
		if (transform) setTransform(shape, transform);
	...
	function setTransform(sprite, transform) {
		...
		if (transform.in != undefined) transform.at != undefined? transform.in.addChildAt(sprite, transform.at) : transform.in.addChild(sprite);
		if (transform.add != undefined) spriteFactory(sprite, transform.add);
		if (transform.dev) dev(sprite);
	}

	function spriteFactory(parent, childs) {
		let i=0;
		while(i < childs.length) {
			if (typeof childs[i] == 'string') {
				childs[i+1].in = parent;
				let sprite = createSprite(childs[i], childs[i+1]);				
				i += 2;
			} else {
				childs[i].in = parent;
				let container = createContainer(childs[i]);
	...
	function dev(sprite) {
		...
		sprite.on('touchstart', mousedown);			
		sprite.on('touchend', mouseup);
		sprite.on('touchendoutside', mouseup);

		function mousedown(e) {
			dx = sprite.x - e.data.global.x/stage.scale.x;
			dy = sprite.y - e.data.global.y/stage.scale.y;
			//console.log(e);
			sprite.on('mousemove', mousemove);
			sprite.on('touchmove', mousemove);
		};
		function mousemove(e) {
			sprite.x = e.data.global.x/stage.scale.x + dx;
			sprite.y = e.data.global.y/stage.scale.y + dy;
		};
		function mouseup() {
			sprite.off('mousemove', mousemove);
			sprite.off('touchmove', mousemove);
			console.log('x:' + sprite.x.toFixed(2) + ', y:' + sprite.y.toFixed(2));
		};
	}
	...
	function ResizeManager() {
		...
		this.find = function(obj) {
			var portrait = data.portrait.find(el=>el[0] == obj);			
			var landscape = data.landscape.find(el=>el[0] == obj);
			return [portrait[1], landscape[1]];
		}

		this.remove = function(obj) {
			var p = data.portrait.find(el=>el[0]==obj);
			var l = data.landscape.find(el=>el[0]==obj);
			data.portrait.splice(data.portrait.indexOf(p),1);
			data.landscape.splice(data.landscape.indexOf(l),1);
		}		
		...
		function resizeHandler() {
			...
			if (data['portrait'].length && iw < ih) {				
				setOrientation('portrait');
			} 
			if(data['landscape'].length && iw > ih) {
				h = app_width;
				w = app_height;
		...
		/*function resizeView(screenWidth, screenHeight) {
