...
function LEV(app_width, app_height) {
	...
	this.createAnimSprite = createAnimSprite;
	this.createAnimSpriteFromJSON = createAnimSpriteFromJSON;
	this.createMesh = createMesh;
	this.loopAnimateMesh = loopAnimateMesh;
	this.setTransform = setTransform;
	this.getGlobalPos = getGlobalPos;	
	this.makeDraggable = makeDraggable;
	this.createShapeRect = createShapeRect;
	...
	/*function createText(txt, font, size, ) {
	...
	function ResizeManager() {
		...
		this.remove = function(obj) {
			...
			data.portrait.splice(data.portrait.indexOf(portrait), 1);
			data.landscape.splice(data.landscape.indexOf(landscape), 1);
		}
		...
		function setOrientation(orient) {
			...
			for (var item of data[orient]) {					
				...
				if (item[3]) item[0].scale.set(item[3], item[3] || item[4]);
			}
		}
