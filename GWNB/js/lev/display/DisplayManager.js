LEV.managers.DisplayManager = function DisplayManager(pixi) {	
	...
	function isIntersectRects(rectA, rectB) {		
		var dx = rectB.x - rectA.x;			
		if (dx > 0 && dx >= rectA.width) return false;
		if (dx < 0 && -dx >= rectB.width) return false;

		var dy = rectB.y - rectA.y;			
		if (dy > 0 && dy >= rectA.height) return false;
		if (dy < 0 && -dy >= rectB.height) return false;
	...
	function DisplaySorterManager() {
		...
		function displaySort(displayA, displayB) {			
			var sortA = displayA.sortY;
			var sortB = displayB.sortY;	
	...
	LEV.components.display = function(props) {
		...
		props.sort = props.sort || {y:0, h:0};				
		display.sortY = props.sort.y;		
		sorter.add(this);
		...
		function onPositionUpdate(p) {
			display.x = p.x;
			display.y = p.y - p.z;
			display.sortY = p.y + p.z + props.sort.y + props.sort.h;			
		}
	...
	LEV.components.debug = function (props) {	
		...
		if (props.sort) {
			if(this.display) {		
				var debugShape = rectShape(-1.5, 0, 3, 3, 0xffff00, 0x000000);
				debugShape.y = -(this.display.sortY - this.position.y - this.position.z);
				this.display.addChild(debugShape);
			} else {
