...
function initLibThree(){
	...
	app.scene3d.background = new THREE.Color(0x8FDCFF);
...
function initLibPixi(){
	...
	app.scene2d.on('pointerout', stageUp);
	app.scene2d.on('pointeroutside', stageUp);
	app.scene2d.on('touchendoutside', stageUp);	

	let clickArea = new PIXI.Container();
	clickArea.interactive = true;	
	clickArea.hitArea = new PIXI.Rectangle(-1280 * 0.5, -1280 * 0.5, 1280, 1280);	
	app.scene2d.addChild(clickArea);
	app.obj2d.clickArea = clickArea;
}
...
function threeActionSetState(name, fadeTime=0.25) {
...
function btnGlobalSound(e){
	if(app.isSounds){
		...
		app.obj2d["objBtnSoundB"].texture = assets.textures.pixi["btn_sound_off"];
		...
		app.obj2d["objBtnSoundB"].texture = assets.textures.pixi["btn_sound_on"];
