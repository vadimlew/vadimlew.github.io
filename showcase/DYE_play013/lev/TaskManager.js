...
function AnimTask({animation, state, finishState, isAsync=true}) {		
	Task.call(this, isAsync);	

	let animFinishHandler = () => {
		animation.mixer.removeEventListener('finished', animFinishHandler);
		if (finishState) animation.set(finishState);
		if (isAsync) this.done();
	}

	animation.mixer.addEventListener('finished', animFinishHandler);
	animation.set(state);
}
...
function MoveTask({obj3d, moveTo, time=1, percent=1, isAsync=true}) {
	...
	let timer = setInterval( ()=>{
		let name = 'Step_' + randomInteger(0, 2);
		if (!assets.sounds[name].playing() )
		playSound( name );		
	}, 250);	
	
	gsap.to(obj3d.position, time, {x, z, ease:'sine.inOut', 		
		onComplete: ()=>{
			this.done();
			clearInterval(timer);
		}
...
function SoundTask({name, isAsync=false}) {
	Task.call(this, isAsync);
