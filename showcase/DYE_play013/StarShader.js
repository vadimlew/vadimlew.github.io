class StarDrawShader extends THREE.ShaderMaterial {	

	constructor( paintTexture, groundTexture, shadowMap ) {
		super(/*{
            lights: true,
        }*/);        

        this.uniforms.paintTexture = { value: paintTexture };
        this.uniforms.groundTexture = { value: groundTexture };

        setTimeout( ()=> {
            console.log( app.lightDirectional.shadow.map.texture );
            this.uniforms.shadowMap = { value: app.lightDirectional.shadow.map.texture };
        }, 500)
        

		app.update.push( this.update.bind(this) );
	}

	uniforms = {
		resolution: { value: new THREE.Vector2(window.innerWidth, window.innerWidth) },
		starTexture: { value: assets.textures.three.star },			
		offset: { value: new THREE.Vector2(0, 0) },
		time: { value: 0 },
        lightPos: { value: app.lightDirectional.position },        
	}
	...
	fragmentShader = `
		...
		void main() {
            vec4 paintColor = texture( paintTexture, vertexUV );

            if ( paintColor.r > 0.1 && paintColor.g < 0.01 && paintColor.b < 0.01) {
                ...
                //vec3 result = (ambient + diffuse) * texture( groundTexture, vertexUV ).xyz;                
                vec3 result = texture( shadowMap, vertexUV ).xyz;                
	...
	update() {
		this.uniforms.offset.value.x += 0.002;
		this.uniforms.offset.value.y += -0.001;		
	}
}
...
class TestMaterial extends THREE.MeshLambertMaterial {
    ...
    onBeforeCompile( shader ) {        
        ...
        `        
        app.update.push( ()=>this.update(shader) );
    }    
