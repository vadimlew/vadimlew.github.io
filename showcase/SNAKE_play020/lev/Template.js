function Template() {
    this.downloadButton = function ({text = 'DOWNLOAD', bgColor = params.colorBtnInstall.value, textColor = params.colorTextBtnInstall.value} = {}) {
        ...
        btnBg.filterShadow = new PIXI.filters.DropShadowFilter();
        btnBg.filterShadow.alpha = 0.1;
        btnBg.filterShadow.angle = 90 * toRAD;
        btnBg.filterShadow.blur = 4;
        btnBg.filterShadow.distance = 10;
        btnBg.filterShadow.color = 0x000000;
        btnBg.filters = [btnBg.filterShadow];
    ...
    this.outlinedText = function({txt='', color=0xffffff, fontSize=50, outlineColor=0x000000, outlineWidth=4, align='center'}) {
        let filterOutline = new PIXI.filters.OutlineFilter(outlineWidth, outlineColor, 3);
    ...
    this.joystick = function({player, layer, maxSpeed=1}) {
        let bgSize = 100;
        let barSize = 35;
        let limit = bgSize - barSize;        
        player.toAngle = player.rotation.y;
        ...
        joystick.dist = 0;
        joystick.angle = 0;
        joystick.limit = limit;

        let joystickBg = new PIXI.Sprite( assets.textures.pixi.joystickBg );
        joystickBg.anchor.set(0.5);
        joystick.addChild(joystickBg);

        let joystickBar = new PIXI.Sprite( assets.textures.pixi.joystickBar );
        joystickBar.anchor.set(0.5);
        joystick.addChild(joystickBar);

        joystick.joystickBar = joystickBar;

        let hand = PIXI.Sprite.from(assets.textures.pixi['hand']);      
        hand.position.set(-8, -8); 
        hand.anchor.set(0.25, 0.0);        
        joystickBar.addChild(hand);

        gsap.to(hand.scale, 0.4, {x:0.8, y:0.8, repeat:-1, yoyo:true, ease:'sine.inOut'});
        ...
        function addedHandler() {
            ...
            layer.on('pointeroutside', stageUpHandler);
            layer.on('touchendoutside', stageUpHandler);  
            joystick.off('added', addedHandler);            
           
            app.update.push(movement);
        }    
        
        function movement() {     
            if (app.stateGame == 'loaded') return;           

            player.rotation.y %= 2*Math.PI;

            let da = player.toAngle - player.rotation.y;            

            if (da > Math.PI) { player.toAngle -= 2*Math.PI };
            if (da < -Math.PI) { player.toAngle += 2*Math.PI };  
            
            if (player.toAngle > player.rotation.y) player.rotation.y += Math.min(0.2, Math.abs(da));
            if (player.toAngle < player.rotation.y) player.rotation.y -= Math.min(0.2, Math.abs(da));            
           
            player.position.x += maxSpeed * Math.sin(player.rotation.y);
			player.position.z += maxSpeed * Math.cos(player.rotation.y);
        }
        ...
        function stageDownHandler(e) {
			...
			mouse.x = e.data.getLocalPosition(joystick.parent).x;
			mouse.y = e.data.getLocalPosition(joystick.parent).y;
        ...
        function stageMoveHandler(e) {
            if (mouse.isDown) {
                ...
                player.toAngle = Math.PI/2 - angle;                
            }
        }
        ...
        joystick.stop = function() {
            ...
            layer.off('pointerout', stageUpHandler);
            layer.off('pointeroutside', stageUpHandler);
            layer.off('touchendoutside', stageUpHandler);            

            stageUpHandler();
        }

        joystick.stopMovement = function() {
            let id = app.update.indexOf(movement);
            app.update.splice(id, 1);
        }
