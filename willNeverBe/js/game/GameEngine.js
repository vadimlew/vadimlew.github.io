...
LEV.game.Engine = function GameEngine(pixi) {
	...
	var garbageTexture = PIXI.RenderTexture.create(1920, 1920);
	var destructPool = {};
	...
	LEV.components.surface = function(props) {
		...
		function onChangePosition(pos) {
			...
			var parent = entity.display.parent;
			var idx = parent.getChildIndex(entity.display);
			for (var obj of objects) {
				parent.setChildIndex(obj.display, idx);
			}
			//trace('surface');
		}
	...
	LEV.components.shooter = function(props) {
		var entity = this;
		var gun = game.factory.weapon[props.gun](entity);		
	...
	LEV.components.handgun = function(props) {
		...
		function update() {
			...
			gun.display.scale.y = sx;

			gun.position.x = shooter.position.x+7.5;
			gun.position.y = shooter.position.y;
			gun.position.z = shooter.position.z+7.5;
			gun.display.sortY = gun.display.y+9*Math.sin(gun.display.rotation) + gun.position.z;
	...
	LEV.components.damage = function(props) {
		...
		function onCollide(collObjB, collObjA) {
			...
			if (kick && collObjB.type != 'obstacle' && entityB.position) {		
				entityB.position.x += entity.phys.body.vx * kick/(entityB.phys? entityB.phys.body.mass : 1);
				entityB.position.y += entity.phys.body.vy * kick/(entityB.phys? entityB.phys.body.mass : 1);
			}
	...
	LEV.components.destructible = function(props) {
		...
		function destructPixels() {				
			...
			var localPos = entity.display.parent.toLocal(bounds);	
			var bottom = localPos.y + parts[parts.length-3] + parts[parts.length-1];			
			...
			for (var i=0; i<parts.length; i+=4) {
				...
				part.x = localPos.x + parts[i];
				part.y = localPos.y + parts[i+1];
