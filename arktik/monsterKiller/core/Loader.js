...
let loader = {
	...
	unzipRead () {
		if (this.zipFiles.length > 0) {
			...
			if (!assets.dataTextures) assets.dataTextures = [];
			if (!assets.dataSounds) assets.dataSounds = [];
			if (!assets.dataModels) assets.dataModels = [];
	...
	loadModels () {
		if (assets.dataModels.length > 0) {
			...
			dracoLoader.setDecoderPath( 'data:application/octet-stream; base64,' );
